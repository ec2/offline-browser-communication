...
describe('connect to one server', () => {
  ...
  it('send messages', async function () {
    ...
    await sleep(3000)
    console.warn(`${((t1 - t0) / numMessages).toFixed(2)}ms (avg over ${numMessages} runs)`);

    expect(res, `sent and received ${numMessages} messages of ${msgToSend.byteLength} bytes each`).to.equal(numMessages)
